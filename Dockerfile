...
FROM scratch
COPY --from=build /out/pamphleteer /pamphleteer
# MIT and BSD licenses require the notices to accompany a distributed binary.
COPY --from=build /src/LICENSE /src/THIRD_PARTY_NOTICES.md /licenses/
USER 65534:65534
ENV VAULT_DIR=/vault LISTEN=:8080
EXPOSE 8080
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD ["/pamphleteer", "-healthcheck"]
ENTRYPOINT ["/pamphleteer"]
